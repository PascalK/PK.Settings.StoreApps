...
Write-Host $Root
$Solution = Get-ChildItem $Root "PK.Settings.StoreApps.sln" -File
...
$PKSettingsStoreApps = Get-ChildItem $Sources "PK.Settings.StoreApps" -Directory
Write-Host $PKSettingsStoreApps.FullName
$PKSettingsStoreAppsProj = Get-ChildItem $PKSettingsStoreApps.FullName "PK.Settings.StoreApps.csproj" -File
Write-Host $PKSettingsStoreAppsProj.FullName

#goto :eof
& $NuGet restore $Solution.FullName

& msbuild /t:Pack /p:Configuration=Release $PKSettingsStoreAppsProj.FullName /p:NuGetExePath=$NuGet